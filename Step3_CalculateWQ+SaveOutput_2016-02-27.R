...
#save(AllVars,LandUse,LandMgmt,Snow,Runoff,SoilP,Pyield,DDL,DDLareal,LakeID,Precip,DDarea,file='AllVarsByLake.Rdata')
load(file='AllVarsByLake.Rdata')

# Remove matrices that will be rebuilt after WQ calculation
rm(LandUse,LandMgmt,Snow,Runoff,SoilP,Pyield,Precip)
...
Year = AllDriverWQ[,1]
Precip = AllDriverWQ[,2:16]
LandUse = AllDriverWQ[,17:24]
LandMgmt = AllDriverWQ[,25:36]
Snow=AllDriverWQ[,37:39]
Runoff=AllDriverWQ[,40:43]
Pyield = AllDriverWQ[,45:47]

# WQ outputs
DDL = AllDriverWQ[,48]
DDLareal = AllDriverWQ[,49]
LakeID = AllDriverWQ[,50]
LULM = AllDriverWQ[,51]
CLIM = AllDriverWQ[,52]
TotLoad = AllDriverWQ[,53]
TP = AllDriverWQ[,54]
Chl = AllDriverWQ[,55]
Secchi = AllDriverWQ[,56]
Hyper = AllDriverWQ[,57]
Pexport = AllDriverWQ[,58]
WQoutput = cbind(LULM,CLIM,LakeID,Year,DDL,DDLareal,TotLoad,TP,Chl,Secchi,Hyper,Pexport)

# Make intuitive column names 

colnames(Precip) = c('AnnPPT','RainSpring','MaxPPT','RainD25','RainD50','RainD75',
                     'MaxNoPPT','MeanDry','MaxDry',
                     'RainW75','RainW100','RainW125','GDD','GSlen','SpringOn')
...
save(AllDriverWQ,Precip,LandUse,LandMgmt,Snow,Runoff,SoilP,Pyield,
